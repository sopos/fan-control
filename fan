...
get_fan_level() {
  debug "reading fan level from '$fan'"
...
get_new_fan_level() {
  ...
  [[ $current_temp -lt $low ]] && {
    log "temperature '$current_temp' dropped below low value '$low', decreasing fan level"
    new_fan_level=$(print_table | grep -B 1 "^$current_fan_level " | head -n 1 | cut -d ' ' -f 1)
  }
  [[ $current_temp -gt $high ]] && {
    log "temperature '$current_temp' exceeded high value '$high', increasing fan level"
